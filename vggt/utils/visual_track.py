...
def visualize_tracks_on_images(
    ...
    for s in range(S):
        ...
        for i in valid_indices:
            ...
            cv2.circle(img_bgr, pt, radius=1, color=color_bgr, thickness=-1)
    ...
    for s in range(S - 1):
        img1_rgb = frame_images[0].copy()
        img2_rgb = frame_images[s + 1].copy()

        tracks_frame1 = tracks[0].numpy()
        tracks_frame2 = tracks[s + 1].numpy()

        img1_bgr_lines = cv2.cvtColor(img1_rgb, cv2.COLOR_RGB2BGR)
        img2_bgr_lines = cv2.cvtColor(img2_rgb, cv2.COLOR_RGB2BGR)

        combined_img_bgr = np.concatenate((img1_bgr_lines, img2_bgr_lines), axis=1)

        for i in range(N):
            is_visible_f1 = True
            is_visible_f2 = True
            if track_vis_mask is not None:
                is_visible_f1 = track_vis_mask[0, i]
                is_visible_f2 = track_vis_mask[1, i]

            if is_visible_f1 and is_visible_f2:
                pt1 = (int(round(tracks_frame1[i, 0])), int(round(tracks_frame1[i, 1])))
                pt2 = (int(round(tracks_frame2[i, 0])), int(round(tracks_frame2[i, 1])))

                R, G, B = track_colors_rgb[i]
                color_bgr = (int(B), int(G), int(R))

                cv2.line(combined_img_bgr, pt1, (pt2[0] + W, pt2[1]), color=color_bgr, thickness=1)
                
                cv2.circle(combined_img_bgr, pt1, radius=1, color=color_bgr, thickness=-1)
                
                cv2.circle(combined_img_bgr, (pt2[0] + W, pt2[1]), radius=1, color=color_bgr, thickness=-1)


        out_path_concat = os.path.join(out_dir, f"frames_00_{s+1:02d}_tracks.png")
        cv2.imwrite(out_path_concat, combined_img_bgr)
        print(
            f"[INFO] Saved concatenated first two frames with tracks -> {out_path_concat}"
        )
