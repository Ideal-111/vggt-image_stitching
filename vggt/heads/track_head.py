...
class TrackHead(nn.Module):
    ...
    def forward(self, aggregated_tokens_list, images, patch_start_idx, query_points=None, iters=None):
        ...
        # 选择第一个批次、第一个时间步的 feature map
        # selected_feature_map = feature_maps[0, 0]
        # visualize_feature_map_for_analysis(selected_feature_map)
...
def visualize_feature_map_for_analysis(feature_map_3d):
    """
    可视化 3D 特征图 (C, H, W)，并为每个通道绘制热力图。
    """
    feature_map_np = feature_map_3d.detach().cpu().numpy()
    num_channels = feature_map_np.shape[0]

    # 根据通道数调整子图布局
    if num_channels > 12:
        print("通道数过多，只显示前12个通道。")
        num_display = 12
    else:
        num_display = num_channels
    ...
    for i in range(num_display):
        ...
        if max_val - min_val > 0:
            ...
            normalized_data = channel_data * 0
        
        ax.imshow(normalized_data, cmap='inferno')
    ...
    plt.suptitle("Feature Map Visualization (Batch 0, Sequence 0)")
